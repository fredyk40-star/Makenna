...
const NumberFishing = ({ game, onComplete, onScoreUpdate, onStarsUpdate }) => {
  ...
  const generateRound = useCallback(() => {
    ...
    const totalFish = 6 + Math.floor(Math.random() * 4);
    const numbers = [target];
    while (numbers.length < totalFish) {
      let num;
      do {
        num = Math.floor(Math.random() * max) + 1;
      } while (numbers.includes(num) || numbers.length >= totalFish);
      numbers.push(num);
    }

    const shuffled = numbers.sort(() => Math.random() - 0.5);
    const newFish = shuffled.map((num, i) => ({
      id: i,
      number: num,
      caught: false,
      x: 5 + Math.random() * 90,

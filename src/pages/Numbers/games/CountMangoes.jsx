...
const CountMangoes = ({ game, onComplete, onScoreUpdate, onStarsUpdate }) => {
  ...
  const generateQuestion = useCallback(() => {
    ...
    const newMangoes = Array.from({ length: total }, (_, i) => ({
      id: i,
      selected: false,
      x: 10 + Math.random() * 80,
      y: 10 + Math.random() * 60,
    ...
    speak(`Count ${target} mangoes`, { rate: 0.7, pitch: 1.1 });
  }, [game.maxNumber, speak]);

  const handleMangoClick = (mangoId) => {
    if (isCorrect !== null || showResult) return;

    setMangoes(prev => prev.map(m => 
      m.id === mangoId ? { ...m, selected: !m.selected } : m
    ));
    ...
    <div className="space-y-4">
      <div className="text-center">
        ...
        <div className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Tap the mangoes to count. You need {targetNumber} mangoes!
        </div>
      ...
      {/* Mango Grid */}
      <div className="relative h-64 bg-gradient-to-br from-yellow-50 to-orange-50 dark:from-gray-700 dark:to-gray-800 rounded-2xl overflow-hidden">
        {mangoes.map((mango) => (
          <motion.button
            key={mango.id}
            initial={{ scale: 0, rotate: -180 }}
            animate={{ scale: 1, rotate: 0 }}
            transition={{ delay: mango.delay, type: 'spring', stiffness: 300 }}
            whileHover={{ scale: 1.15 }}
            whileTap={{ scale: 0.85 }}
            onClick={() => handleMangoClick(mango.id)}
            disabled={isCorrect !== null || showResult}
            className={`absolute text-4xl transition-all duration-300 ${
              mango.selected ? 'opacity-100 scale-110' : 'opacity-90 hover:scale-110'
            }`}
            style={{ left: `${mango.x}%`, top: `${mango.y}%` }}
            aria-label={mango.selected ? 'Selected mango' : 'Mango'}
          >
            🥭
            {mango.selected && (
              <motion.div

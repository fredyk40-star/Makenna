...
import { generateSubtractionProblem } from '../../data/mathsData';
...
const Subtraction = () => {
  ...
  const generateNewProblem = useCallback(() => {
    const newProblem = generateSubtractionProblem(20);
    ...
    setTimeout(() => {
      speak(`What is ${newProblem.a} minus ${newProblem.b}?`, { rate: 0.7, pitch: 1.1 });
    }, 300);
  }, [speak]);
  ...
  const handleAnswer = (answer) => {
    ...
    if (correct) {
      ...
      adaptiveLearningService.recordPerformance('maths_subtraction', {
        correct: 1,
        incorrect: 0,
        time: 2,
        difficulty: 'easy'
  ...
  // Visual subtraction - show objects being removed
  const renderObjects = (count, removeCount) => {
    const emojis = ['⭐', '🔵', '🟢', '🔴'];
    const emoji = emojis[Math.floor(Math.random() * emojis.length)];
    const items = [];
    for (let i = 0; i < count; i++) {
      const isRemoved = i >= count - removeCount;
      items.push(
        <span 
          key={i} 
          className={`text-2xl transition-all duration-500 ${isRemoved ? 'opacity-0 scale-50' : 'opacity-100'}`}
        >
          {emoji}
        </span>
      );
  ...
  return (
    ...
    >
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-soft p-6 text-center">
        ...
        {/* Visual Counters with removal animation */}
        <div className="mt-4 flex flex-wrap justify-center gap-2 min-h-[60px]">
          {renderObjects(problem.a, problem.b)}
        </div>

        <div className="mt-2 text-sm text-gray-500 dark:text-gray-400">
          {problem.b} objects removed
        </div>

        <button
          onClick={() => speak(`${problem.a} minus ${problem.b} equals?`)}
          className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
        >
      ...
      <div className="grid grid-cols-2 gap-3 max-w-md mx-auto">
        {problem.options.map((option) => (
          <motion.button
            ...
            className={`p-4 rounded-xl text-2xl font-bold transition-all duration-300 ${
            ...
            } ${selectedAnswer !== null && selectedAnswer !== option && option !== problem.answer ? 'opacity-50' : ''}`}
          >
            {option}
          </motion.button>
        ))}

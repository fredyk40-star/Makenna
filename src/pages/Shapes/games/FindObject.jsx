...
const FindObject = ({ game, onComplete, onScoreUpdate, onStarsUpdate }) => {
  ...
  const generateQuestion = useCallback(() => {
    ...
    speak(`Find the object that is a ${target.name}`, { rate: 0.7, pitch: 1.1 });
    announceToScreenReader(`Find the ${target.name} object`);
  }, [speak]);
    ...
    <div className="space-y-4">
      <div className="text-center">
        ...
        <div className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Match the shape to the real object!
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 max-w-md mx-auto">
        {options.map((shape) => (
          ...
          >
            <div className="text-5xl">{shape.objectEmoji}</div>
            <div className="mt-2 font-semibold">{shape.realObject}</div>
